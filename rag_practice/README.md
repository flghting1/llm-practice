...
本次新增 7 份根据公开岗位信息整理的去标识化 JD 摘要。检索准确率保持不变，但文档数量增加后平均响应时间上升，原因是当前实现会在每次问题检索时重新计算全部文档 Embedding。
...
已在 `embedding_retriever.py` 中加入进程内文档向量缓存。首次查询包含模型和向量初始化，后续查询复用缓存，单题响应时间明显下降。
